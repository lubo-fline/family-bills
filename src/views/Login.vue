 ...
 * @LastEditTime: 2022-08-09 11:04:27
...
<template>
    <div class="fl_login">
        <div id="fl_loginBg"></div>
        <div class="fl_loginFormBox">
            <div class="fl_loginContainer">
                <img
                    class="fl_human"
...
const onFinish = (values: any) => {
    proxy.$post(proxy.$api.oauthToken.login, values).then((res: any) => {
        if (res.retCode === 0) {
...
onMounted(() => {
    container = document.getElementById("fl_loginBg");
    ...
    return () => {
        ...
        if (cloudProgress > 1) {
            ...
            if (cloudParameter.curve) {
                ...
                if (point && point.x) {
                    cloudParameter.cloud.position.set(
                        point.x,
                        point.y,
                        point.z
                    );
