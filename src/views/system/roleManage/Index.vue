...
-->
<template>
    <a-card :bordered="false" size="small" title="角色管理">
        <a-form
            :model="formState"
    ...
    <addModal :visible="visible" @handleCancel="handleCancel"></addModal>
    <editModal :visible="visible" :editData="dataSource.editData" @handleCancel="handleCancel"></editModal>
...
<script setup lang='ts'>
    ...
    const getTableData=()=>{
        ...
        proxy.$post(proxy.$api.system.role.list,params).then((res:any)=>{
            loading.value=false
            if(res.retCode===0){
    ...
    const deleteItem=(id:number)=>{
        Modal.confirm({
            ...
            content: '是否要删除该数据？',
            onOk() {
                proxy.$del(proxy.$api.system.role.delete+id).then((res:any)=>{
                    if(res.retCode===0){
    ...
    const resetItem=(id:number)=>{
        Modal.confirm({
            ...
            content: '是否要启用该数据？',
            onOk() {
                proxy.$put(proxy.$api.system.role.reset+id).then((res:any)=>{
                    if(res.retCode===0){
